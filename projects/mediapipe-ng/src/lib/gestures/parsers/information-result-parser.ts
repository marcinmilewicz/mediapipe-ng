...
export const informationResultParser = <Type = GestureType>(
  results: GestureRecognizerResult,
): InformationResult<Type> => {
  if (!results.gestures.length) {
    return { categoryName: 'None', categoryScore: 100, description: '' };
  }

  const categoryName = results.gestures[0][0].categoryName as Type;
  const categoryScore = results.gestures[0][0].score * 100;
  const handedness = results.handedness[0][0].displayName as 'Right' | 'Left';
